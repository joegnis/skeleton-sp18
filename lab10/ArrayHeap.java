...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        int parentIndex;
        while (index > 1 && contents[index].priority() < contents[(parentIndex = parentIndex(index))].priority()) {
    ...
    private void sink(int index) {
        ...
        int leftIndex;
        while ((leftIndex = leftIndex(index)) <= size) {
            int rightIndex = rightIndex(index);
            // Finds the min of the two children
            // since we made sure leftIndex <= size, we won't have two null as children
            int minChild = min(leftIndex, rightIndex);
            if (contents[index].priority() <= contents[minChild].priority()) {
                break;
            }
            swap(index, minChild);
            index = minChild;
    ...
    public void insert(T item, double priority) {
        ...
        size++;
        // swim will check bounds, so we need to change size first
        swim(size);
    }
    ...
    public void testChangePriority() {
        ...
        assertEquals("a", pq.contents[1].myItem);
        assertEquals("d1", pq.contents[2].myItem);
        assertEquals("e", pq.contents[3].myItem);
        assertEquals("c", pq.contents[4].myItem);
